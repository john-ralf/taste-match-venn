...
async function searchSpotify(kind: MusicKind, query: string, env: Env): Promise<SearchResult> {
  try {
...
async function searchLastFmTracks(query: string, env: Env): Promise<SearchResult> {
  ...
  try {
    ...
    const items = await Promise.all(tracks.map((track) => mapLastFmTrack(track, env.LASTFM_API_KEY as string)));
...
function sanitizeItems(items: unknown, kind: MusicItem["kind"]): MusicItem[] {
  ...
  return items
    .filter((item) => isRecord(item) && (item.source === "spotify" || item.source === "lastfm"))
    ...
    .map((item, index) => {
      ...
      const itemSource = source.source === "lastfm" ? "lastfm" : "spotify";
